...
class BasicDataset(Dataset):
    def __init__(self, configs,  state: str = 'train', scale: float = 1.0, mask_suffix: str = ''):
    ...
    def __getitem__(self, idx):
        ...
        assert img.size == mask[0].size, \
            f'Image and mask {name} should be the same size, but are {img.size} and {mask.size}'

        img = self.preprocess(img, self.scale, is_mask=False)
        # mask = self.preprocess(mask, self.scale, is_mask=True)

        mask0 = self.preprocess(mask[0], self.scale, is_mask=True)
        mask1 = self.preprocess(mask[1], self.scale, is_mask=True)
        mask2 = self.preprocess(mask[2], self.scale, is_mask=True)
        mask3 = self.preprocess(mask[3], self.scale, is_mask=True)
        mask4 = self.preprocess(mask[4], self.scale, is_mask=True)
        mask5 = self.preprocess(mask[5], self.scale, is_mask=True)
        mask6 = self.preprocess(mask[6], self.scale, is_mask=True)
        mask7 = self.preprocess(mask[7], self.scale, is_mask=True)
        mask8 = self.preprocess(mask[8], self.scale, is_mask=True)
